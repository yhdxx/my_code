...
class FederatedClient:
    ...
    def train_shared_layers_and_get_gradients(self):
        ...
        for data, target in self.train_loader:
            ...
            # 收集共享层的梯度并转换为服务器模型兼容的键名
            for i, layer in enumerate(self.model.shared_layers):
                if hasattr(layer, 'weight') and layer.weight.grad is not None:
                    server_key = f'layers.{2 * i}.weight'
                    gradients_dict[server_key] = layer.weight.grad.clone()

                if hasattr(layer, 'bias') and layer.bias is not None and layer.bias.grad is not None:
                    server_key = f'layers.{2 * i}.bias'
                    gradients_dict[server_key] = layer.bias.grad.clone()
